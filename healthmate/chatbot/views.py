...
def search_conversation_history(request):
    if request.method == 'POST':
        ...
        conversations = ConversationHistory.objects.filter(session_id=session_id)
        for convo in conversations:
            if search_keyword in convo.user_message.lower() or search_keyword in convo.bot_response.lower():
